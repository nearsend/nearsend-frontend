...
class NumberFormat extends React.Component<any> {
  ...
  formatAsNumber(numStr: string) {
    const { decimalScale, fixedDecimalScale, prefix, suffix, allowNegative, thousandsGroupStyle, allowTrailingZeros } =
      this.props;
    ...
    if (!allowTrailingZeros) afterDecimal = removeTrailingZero(afterDecimal);
    ...
    numStr =
      beforeDecimal +
      ((hasDecimalSeparator && afterDecimal.replace(suffix, '') && decimalSeparator) || '') +
      afterDecimal;
  ...
  onBlur(e: SyntheticInputEvent) {
    ...
    const { format, onBlur, allowLeadingZeros, allowTrailingZeros } = props;
    ...
    if (!format) {
      ...
      if (!allowTrailingZeros) {
        numAsString = fixTrailingZero(numAsString);
      }
